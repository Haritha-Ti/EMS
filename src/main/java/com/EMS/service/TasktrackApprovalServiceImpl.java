			...
			if (approvalUserList != null && approvalUserList.size() > 0) {
				...
				for (TaskTrackApproval item : approvalUserList) {
					...
					if (item.getProjectType().equalsIgnoreCase("Billable")) {
						...
						secondHalfStatus = item.getSecondHalfStatus();
						String status = null;
...
//					taskTrackApproval.setApprovedDate(endDate);
	...
	@Override
	public JSONObject getDataForApprovalLevelTwo(Long userId, Date startDate, Date endDate,
			Long projectId, Integer firstHalfDay) {
		// TODO Auto-generated method stub
