...
public class ProjectReportsRepository extends DbConnectionUtility {
	...
	/*
	 * public List<JsonNode> GenerateProjectReports (){ String sql =
	 * "SELECT CONCAT(u.first_name,' ',u.last_name) AS users , u.qualification, a.is_billable, a.allocated_perce, a.start_date, a.end_date, p.project_name FROM allocation a LEFT JOIN `user` u ON u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id = a.project_project_id WHERE CAST(a.start_date  AS DATE) >= '2019/04/01' AND CAST(a.end_date  AS DATE)  <=  '2019/04/30' "
	 * ; List<JsonNode> list = jdbcTemplate.query(sql, new JsonNodeRowMapper(new
	 * ObjectMapper())); return list; }
	 */
	public List<ProjectReportModel> GenerateProjectReports(long projectId, Date fromDate, Date toDate) {
		...
		if (projectId == 0) {
			// sql = "SELECT CONCAT(u.first_name,' ',u.last_name) AS users , a.is_billable,
			// a.allocated_perce, p.project_name FROM allocation a LEFT JOIN `user` u ON
			// u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id =
			// a.project_project_id WHERE CAST(a.start_date AS DATE) >= CAST(? AS DATE) AND
			// CAST(a.end_date AS DATE) <= CAST(? AS DATE) order by project_name ";
			sql = "SELECT CONCAT(u.last_name,' ',u.first_name) AS users ,  a.is_billable, a.allocated_perce, p.project_name,u.region_id FROM allocation a LEFT JOIN `user` u ON u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id = a.project_project_id WHERE (date(?) between date(a.start_date) and date(a.end_date) or date(?) between date(a.start_date) and date(a.end_date))  order by project_name";
			list = jdbcTemplate.query(sql, new ReportRowMapper(), new Object[] { fromDate, toDate });
		} else {
			// sql = " SELECT CONCAT(u.first_name,' ',u.last_name) AS users , a.is_billable,
			// a.allocated_perce, p.project_name FROM allocation a LEFT JOIN `user` u ON
			// u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id =
			// a.project_project_id WHERE p.project_id = ? AND CAST(a.start_date AS DATE) >=
			// CAST(? AS DATE) AND CAST(a.end_date AS DATE) <= CAST(? AS DATE) order by
			// project_name";

			// Changed by Haritha 21-10-2019//
			// sql = "SELECT CONCAT(u.last_name,' ',u.first_name) AS users , a.is_billable,
			// a.allocated_perce, p.project_name FROM allocation a LEFT JOIN `user` u ON
			// u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id =
			// a.project_project_id WHERE p.project_id = ? AND ((CAST(a.start_date AS DATE)
			// >= CAST(? AS DATE) AND CAST(a.start_date AS DATE) <= CAST(? AS DATE) ) OR
			// (CAST(a.end_date AS DATE) >= CAST(? AS DATE) AND CAST(a.end_date AS DATE) <=
			// CAST(? AS DATE) ) OR (CAST(a.start_date AS DATE) >= CAST(? AS DATE) AND
			// CAST(a.end_date AS DATE) <= CAST(? AS DATE) ) ) order by project_name";
			// list = jdbcTemplate.query(sql, new ReportRowMapper(), new Object[]
			// {projectId,fromDate,toDate,fromDate,toDate,fromDate,toDate});
			// Changed by Haritha 21-10-2019//

			sql = "SELECT CONCAT(u.last_name,' ',u.first_name) AS users ,  a.is_billable, a.allocated_perce, p.project_name , u.region_id FROM allocation a LEFT JOIN `user` u ON u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id = a.project_project_id WHERE p.project_id = ? AND (date(?) between date(a.start_date) and date(a.end_date) or date(?) between date(a.start_date) and date(a.end_date)) order by project_name";
	...
	public List<BenchProjectReportModel> GenerateBenchProjectReports(Long uId, Date fromDate, Date toDate) {
		...
		// sql = "SELECT CONCAT(u.first_name,' ',u.last_name) AS users ,u.user_id,
		// a.is_billable, a.allocated_perce, p.project_name, d.department_id,
		// d.department_name FROM allocation a LEFT JOIN `user` u ON u.user_id = a.
		// user_user_id LEFT JOIN project p ON p.project_id = a.project_project_id LEFT
		// JOIN department d ON d.department_id = u.department_department_id WHERE
		// p.project_code = 'BP' AND u.user_id=? AND CAST(a.start_date AS DATE) >=
		// CAST(? AS DATE) AND CAST(a.end_date AS DATE) <= CAST(? AS DATE) order by
		// users";
		...
		list = jdbcTemplate.query(sql, new BenchReportRowMapper(),
				new Object[] { uId, fromDate, toDate, fromDate, toDate, fromDate, toDate });
	...
	public List<ExportProjectTaskReportModel> GenerateProjectTaskReportForExporting(Date fromDate, Date toDate,
			Long projectId) {
		String sql = "";
		List<ExportProjectTaskReportModel> list = null;

		// sql = " SELECT id,CAST(`date` AS DATE) AS taskDate,CONCAT(u.first_name,'
		// ',u.last_name) AS resourceName, `description` as taskDescription,hours ,
		// p.project_name as projectName,p.project_id as projectId, (CASE WHEN
		// a.is_billable IS NULL THEN '-' WHEN a.is_billable = '1' THEN 'Yes' ELSE 'No'
		// END) AS billable from tasktrack t LEFT JOIN `user` u ON u.user_id = t.
		// user_user_id LEFT JOIN allocation a ON a.project_project_id =
		// t.project_project_id and a.user_user_id = t.user_user_id LEFT JOIN project p
		// on p.project_id = t.project_project_id where CAST(`date` AS DATE) BETWEEN ?
		// AND ? and t.project_project_id = ? order by TaskDate,ResourceName,hours";
		sql = "SELECT DISTINCT CAST(`date`  AS DATE) AS taskDate,CONCAT(u.last_name,'',u.first_name) AS resourceName,tm.task_name as taskCategory,  `description` as taskDescription,hours , p.project_name as projectName,  (CASE 	WHEN a.is_billable IS NULL THEN '-' 	WHEN a.is_billable = '1' THEN 'Yes' 	ELSE 'No' END) AS billable  from tasktrack t  LEFT JOIN `user` u ON  u.user_id = t. user_user_id  LEFT JOIN allocation a ON a.project_project_id = t.project_project_id and a.user_user_id = t.user_user_id  LEFT JOIN project p on p.project_id = t.project_project_id  LEFT JOIN task_master tm on tm.id = t.task_id  where CAST(`date`  AS DATE) >= CAST(?  AS DATE)  AND CAST(`date`  AS DATE)  <=  CAST(?  AS DATE) and t.project_project_id = ? order by TaskDate,ResourceName,hours";
		list = jdbcTemplate.query(sql, new ExportProjectTaskReportRowMapper(),
				new Object[] { fromDate, toDate, projectId });
	...
	public List<ApprovalTimeTrackReportModel> getApprovalStatusReportDetails(Date startDate, Date endDate,
		...
		System.out.println("startdateIndex==" + startdateIndex + "  enddateIndex==" + enddateIndex + "  month==" + month
				+ "  year==" + year);
		StringBuffer buffer = new StringBuffer();

		buffer.append(
				"SELECT ems1.projectName,ems1.projectId,COALESCE(SUM(ems1.approved),0) AS 'approved', SUM(ems1.logged) AS 'logged' FROM ( SELECT ems.projectName,ems.projectId,CASE WHEN ems.label = 'approved' THEN hours END AS 'approved', CASE WHEN ems.label = 'logged' THEN hours END AS 'logged' FROM ( SELECT p.project_name AS projectName,SUM(");
		...
		buffer.append(
				" 'approved' AS label,p.project_id AS projectId FROM tasktrack_approval_final ta LEFT JOIN `user` u ON u.user_id = ta.user_user_id LEFT JOIN project p ON p.project_id = ta.project_project_id WHERE ta.month=? AND ta.year=? AND ta.project_type ='Billable' GROUP BY 1,3,4 UNION  SELECT p.project_name AS projectName,SUM(tt.hours) AS hours,'logged' AS label,p.project_id AS projectId FROM tasktrack tt LEFT JOIN `user` u ON u.user_id = tt.user_user_id LEFT JOIN project p ON p.project_id = tt.project_project_id WHERE CAST(tt.`date`  AS DATE) >= CAST(?  AS DATE)  AND CAST(tt.`date`  AS DATE)  <=  CAST(?  AS DATE) GROUP BY 1,3,4 ) ems )ems1 GROUP BY 1,2");

		// sql="SELECT ems1.projectName,COALESCE(SUM(ems1.approved),0) AS 'approved',
		// SUM(ems1.logged) AS 'logged' FROM ( SELECT ems.projectName,CASE WHEN
		// ems.label = 'approved' THEN hours END AS 'approved', CASE WHEN ems.label =
		// 'logged' THEN hours END AS 'logged' FROM ( SELECT p.project_name AS
		// projectName,SUM(COALESCE(day1,0)+COALESCE(day2,0)+COALESCE(day3,0)+COALESCE(day4,0)+COALESCE(day5,0)+COALESCE(day6,0)+COALESCE(day7,0)+COALESCE(day8,0)+COALESCE(day9,0)+COALESCE(day10,0)
		// +COALESCE(day11,0)+COALESCE(day12,0)+COALESCE(day13,0)+COALESCE(day14,0)+COALESCE(day15,0)+COALESCE(day16,0)+COALESCE(day17,0)+COALESCE(day18,0)+COALESCE(day19,0)+COALESCE(day20,0)
		// +COALESCE(day21,0)+COALESCE(day22,0)+COALESCE(day23,0)+COALESCE(day24,0)+COALESCE(day25,0)+COALESCE(day26,0)+COALESCE(day27,0)+COALESCE(day28,0)+COALESCE(day29,0)+COALESCE(day30,0)
		// +COALESCE(day31,0)) AS hours, 'approved' AS label FROM tasktrack_approval ta
		// LEFT JOIN `user` u ON u.user_id = ta.user_user_id LEFT JOIN project p ON
		// p.project_id = ta.project_project_id WHERE ta.month=? AND ta.year=? AND
		// ta.project_type ='Billable' GROUP BY 1,3 UNION SELECT p.project_name AS
		// projectName,SUM(tt.hours) AS hours,'logged' AS label FROM tasktrack tt LEFT
		// JOIN `user` u ON u.user_id = tt.user_user_id LEFT JOIN project p ON
		// p.project_id = tt.project_project_id WHERE CAST(tt.`date` AS DATE) BETWEEN ?
		// AND ? GROUP BY 1,3 ) ems )ems1 GROUP BY 1";
		sql = buffer.toString();
		System.out.println("#############################");
		System.out.println(sql);
		list = jdbcTemplate.query(sql, new ApprovalTimeTrackReportRowMapper(),
				new Object[] { month, year, startDate, endDate });

		return list;
	}

	public List<ExportProjectHourReportModel> GenerateProjectHourReportForExporting(Date fromDate, Date toDate,
		...
		list = jdbcTemplate.query(sql, new ExportProjectHourReportRowMapper(),
				new Object[] { monthIndex, yearIndex, fromDate, toDate });

		return list;
	}

	public Object GenerateProjectReportsByRegion(Long projectId, Date fromDate, Date toDate, Long regionIdSelected) {
		...
		if (projectId == 0) {
			...
			// a.project_project_id WHERE CAST(a.start_date AS DATE) >= CAST(? AS DATE) AND
			// CAST(a.end_date AS DATE) <= CAST(? AS DATE) order by project_name ";
			sql = "SELECT CONCAT(u.last_name,' ',u.first_name) AS users ,  a.is_billable, a.allocated_perce, p.project_name,u.region_id FROM allocation a LEFT JOIN `user` u ON u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id = a.project_project_id WHERE (date(?) between date(a.start_date) and date(a.end_date) or date(?) between date(a.start_date) and date(a.end_date))  AND u.region_id = ? order by project_name";
			list = jdbcTemplate.query(sql, new ReportRowMapper(), new Object[] { fromDate, toDate, regionIdSelected });
		} else {
			// sql = " SELECT CONCAT(u.first_name,' ',u.last_name) AS users , a.is_billable,
			// a.allocated_perce, p.project_name FROM allocation a LEFT JOIN `user` u ON
			// u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id =
			...
			sql = "SELECT CONCAT(u.last_name,' ',u.first_name) AS users ,  a.is_billable, a.allocated_perce, p.project_name , u.region_id FROM allocation a LEFT JOIN `user` u ON u.user_id = a. user_user_id LEFT JOIN project p ON p.project_id = a.project_project_id WHERE p.project_id = ? AND (date(?) between date(a.start_date) and date(a.end_date) or date(?) between date(a.start_date) and date(a.end_date))  AND u.region_id = ?  order by project_name";
			list = jdbcTemplate.query(sql, new ReportRowMapper(),
					new Object[] { projectId, fromDate, toDate, regionIdSelected });
		}
		return list;
	}
	...
	public List<BenchProjectReportModel> GenerateBenchProjectReports(Date fromDate, Date toDate, Long regionId) {
		...
		list = jdbcTemplate.query(sql, new BenchReportRowMapper(),
				new Object[] { fromDate, toDate, fromDate, toDate, fromDate, toDate, regionId });
	...
	public List<FreeAllocationReportModel> getFreeAllocationReportList(Date fromDate, Date toDate,
		...
		list = jdbcTemplate.query(sql, new FreeAllcoationReportRowMapper(), new Object[] { fromDate, toDate, fromDate,
				toDate, toDate, toDate, fromDate, fromDate, regionId_selected });
		return list;
	}
