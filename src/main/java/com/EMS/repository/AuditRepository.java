...
@Query(value="select sm.semi_monthly_id ,sm.trx_date , sm.day1 ,sm.day10 ,sm.day11 ,sm.day12 ,sm.day13 ,sm.day14 ,sm.day15 ,sm.day16 ,sm.day17, "
	...
	+ " then 'Consultant' "
	+ " when (sm.user_in_action = sm.first_half_approver_one_user_id ) or (sm.user_in_action = sm.second_half_approver_one_user_id) "
	+ " then 'Approver 1' "
	+ " when (sm.user_in_action = sm.first_half_approver_two_user_id ) or (sm.user_in_action = sm.second_half_approver_two_user_id) "
	+ " then 'Approver 2' "
	+ " when (sm.user_in_action = sm.first_half_finance_user_id ) or (sm.user_in_action = sm.second_half_finance_user_id) "
	+ " then 'Finance' "
	+ " else 'Watcher' "
	+ " end as role "
	+ " from  tasktrack_approval_semimonthly_aud sm "
	+ " left JOIN user ua ON sm.user_in_action=ua.user_id "
	+ " where sm.semi_monthly_id=?1 and sm.submission_period =?2 "
	...
	+ " then 'Consultant' "
	+ " when wk.user_in_action = wk.approver1_user_id "
	+ " then 'Approver 1'"
	+ " when wk.user_in_action = wk.approver2_user_id "
	+ " then 'Approver 2' "
