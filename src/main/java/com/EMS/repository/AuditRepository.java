...
@Query(value="SELECT u.user_name,p.project_name,t.revtype,DATE_FORMAT(STR_TO_DATE(u.trx_date, '%m/%d/%Y %h:%i%p'),'%Y-%m-%d'),t.user_in_action,\r\n" + 
		...
		//+ "and DATE_FORMAT(u.trx_date,'%Y-%m-%d') >=?3 and  DATE_FORMAT(u.trx_date,'%Y-%m-%d') <=?4 \r\n" 
		+ 
...
@Query(value=" SELECT  case  when aud.revtype=0 then 'Created'  when aud.revtype=1 then 'Updated'  when aud.revtype=2 then 'Deleted' else null end as revtype ,"
		...
		+ " case when  aud.ispoc=1 then 'Ic' when aud.ispoc=0 then 'Non-Ic' else null end as ispoc  , "
		+ " pp.project_name as parent_project,aud.project_category ,aud.project_code ,aud.project_details ,aud.project_name ,"
		...
		+ " aud.project_type  , DATE_FORMAT(aud.releasing_date , '%d/%m/%Y') as releasing_date , DATE_FORMAT(aud.start_date, '%d/%m/%Y') as start_date ,c.client_name ,ct.contract_type_name  ,concat(olu.first_name,' ',olu.last_name) as onsite_lead , concat(pwu.first_name,' ',pwu.last_name) as project_owner"
