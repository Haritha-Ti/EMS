...
public interface TaskTrackFinalJPARepository extends JpaRepository<TaskTrackApprovalFinal, Long> {
   ...
   /* @Query(value ="SELECT user.user_id as id," +
            ...
            "sum(COALESCE(day31,0)) as day31 FROM tasktrack_approval_final where user_user_id=?3 and month=?1 and year=?2 and project_type in('Billable','Overtime','Non-Billable')",nativeQuery = true)*/
