...
public interface UserRepository extends JpaRepository<UserModel, Long> {
	...
	@Query("SELECT u FROM UserModel u WHERE u.userName=?1 AND u.password=?2 AND u.active = true") // Query for checking
																									// username and
																									// password are
																									// matching or not
//	@Query("SELECT u FROM UserModel u WHERE u.userName=?1 AND u.active = true") 
	...
	@Query("SELECT u FROM UserModel u WHERE u.role in (11,8)") // for getting user details with role as owner by
																// providing role
...
//	@Query("select u from UserModel u where u.userName = ?1")
	...
	@Query("SELECT u FROM UserModel u WHERE u.role in (11,3,5) AND department_department_id in('1','2','3','4','8','6','5','7')   order by u.lastName")
	...
	@Query("Select u From UserModel u where u.region.id = ?1 and  (u.terminationDate >= ?2 or u.terminationDate IS NULL) and u.joiningDate <=?2 and  u.role.roleId in('2','3','5','11','9') and u.department.departmentId in('1','2','3','4','8')")
	// @Query("Select u From UserModel u where u.region.id = ?1 and
	// (u.terminationDate >= ?2 or u.terminationDate IS NULL) and u.joiningDate <=?2
	// and u.role.roleId not in('1') ")
	List<UserModel> getUsersBasedOnMonthYearRegion(Long regionId, Date date1);
	...
	// @Query("Select u From UserModel u where (u.terminationDate >= ?1 or
	// u.terminationDate IS NULL) and u.joiningDate <=?1 and u.role.roleId not
	// in('1')")
