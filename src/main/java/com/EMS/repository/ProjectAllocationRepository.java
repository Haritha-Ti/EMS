...
//	}
	...
	List<AllocationModel> findUsers(@Param("userId") long userId, @Param("date1") Date date1,
...
//	List<Object[]> getUserIdByProject(Long projectId, Long pageSize, Long limit);
	...
	// @Query("select u.user.userId from AllocationModel u where u.project.projectId
	// = ?1 and ((u.startDate between ?2 and ?3) OR (u.endDate between ?2 and ?3))
	// and u.user.active = true and u.active = true")
	...
	@Query("select u from AllocationModel u where u.project.projectId = ?1 and u.startDate <=?3 and u.endDate >=?2 ")
...
//	List<Alloc> findUsers(long userId, Date date1, Date date2);
	...
	@Query(value = "SELECT allocation.alloc_id FROM allocation WHERE allocation.project_project_id = ?1 "
			+ " and allocation.user_user_id = ?2 " + " order by allocation.end_date desc "
	...
	@Query(value = "select distinct p.project_id, p.project_name,c.client_name, alc.start_date, alc.end_date "
			+ "from allocation alc " + "join project p on p.project_id  = alc.project_project_id "
			+ "left join client c on c.client_id = p.client_name_client_id "
			+ "join user usr on usr.user_id = alc.user_user_id "
			+ "left join tasktrack t on t.project_project_id = p.project_id and t.user_user_id = usr.user_id "
			+ "where alc.user_user_id= ?1 "
			+ "and (?2 between alc.start_date and alc.end_date or ?3 between alc.start_date and alc.end_date or "
			+ "(?2 <= alc.start_date and ?3 >= alc.end_date)) " + "order by 1,2,3,4,5", nativeQuery = true)
